...
class FarePriceController extends Controller
{
    ...
    public function calculateDistances($origins = null, $destinations = null)
    {
    ...
    public function calculateFarePrice(CalculateFarePriceRequest $request)
    {
        ...
        try {
            ...
            if ($request->apply_filter == 0) {
                ...
                $distance_information = $this->calculateDistances($origins, $destinations);
            ...
            $vehicles = Vehicle::join('vehicle_ride_charge_state_wises', function ($vehicle_ride_charge_state_wises) {
            ...
            if ($request->save_location && $vehicles->count() > 0) {
                ...
                $this->saveFareHistory($points, $vehicles, auth()->id());
            }
    ...
    public function saveFareHistory($points,  $data, $user_id)
    {
        FareHistory::create(['data' => $data, 'points' => json_encode($points), 'user_id' => $user_id]);
    }
    ...
    public function fareHistory(Request $request)
    {
        ...
        $history = FareHistory::where('user_id', auth()->id())
            ->when($location, function ($query) use ($location) {
                return $query->whereRaw("(CONVERT(points USING utf8) LIKE '%" . $location . "%')");
            })
